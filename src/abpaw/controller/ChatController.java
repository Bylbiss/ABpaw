...
public class ChatController {
    ...
    public boolean sendMessageFromPemilik(int idPemilik, int idDokter, String pesan) {
        Chat chat = new Chat();
        chat.setIdPemilik(idPemilik); // pengirim
        chat.setIdDokter(idDokter);   // penerima
        chat.setPesan(pesan);
    ...
    public boolean sendMessageFromDokter(int idDokter, int idPemilik, String pesan) {
        Chat chat = new Chat();
        chat.setIdDokter(idDokter);   // pengirim
        chat.setIdPemilik(idPemilik); // penerima
        chat.setPesan(pesan);
        chat.setStatusBaca("read");   // dokter sendiri, tidak perlu unread
        chat.setWaktu(new Timestamp(System.currentTimeMillis()));
        return chatDAO.insert(chat);
    }
