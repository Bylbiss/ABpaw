...
public class PemilikDAO implements CRUDable<Pemilik> {
        ...
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (affected > 0) {
                try (ResultSet rs = stmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        pemilik.setIdPemilik(rs.getInt(1));
                    }
                }
                return true;
            }
    ...
    public boolean isEmailExistExcept(String email, int idPemilik) {
        String sql = "SELECT 1 FROM pemilik WHERE email = ? AND id_pemilik != ?";
