...
public class PemesananDAO {
    ...
    public boolean insertOffline(PemesananOffline pesan) {
        String sql = "INSERT INTO pemesanan_offline (id_dokter, id_pemilik, id_pet, nomor_antrean, tanggal_antrean, waktu_antrean, keluhan, status_antrean, estimasi_waktu, biaya_jasa, total_biaya, status_pembayaran) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            stmt.setBigDecimal(10, pesan.getBiayaJasa() != null ? pesan.getBiayaJasa() : BigDecimal.ZERO);
            stmt.setBigDecimal(11, pesan.getTotalBiaya() != null ? pesan.getTotalBiaya() : BigDecimal.ZERO);
            stmt.setString(12, pesan.getStatusPembayaran() != null ? pesan.getStatusPembayaran() : "belum_bayar");
            int affected = stmt.executeUpdate();
            if (affected > 0) {
                try (ResultSet rs = stmt.getGeneratedKeys()) {
                    if (rs.next()) pesan.setIdTransaksi(rs.getInt(1));
    ...
    public boolean updateOffline(PemesananOffline pesan) {
        String sql = "UPDATE pemesanan_offline SET id_dokter=?, id_pemilik=?, id_pet=?, nomor_antrean=?, " +
                     "tanggal_antrean=?, waktu_antrean=?, keluhan=?, status_antrean=?, estimasi_waktu=?, " +
                     "biaya_jasa=?, total_biaya=?, status_pembayaran=? WHERE id_antrean=?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, pesan.getIdDokter());
            stmt.setInt(2, pesan.getIdPemilik());
            ...
            stmt.setBigDecimal(11, pesan.getTotalBiaya() != null ? pesan.getTotalBiaya() : BigDecimal.ZERO);
            stmt.setString(12, pesan.getStatusPembayaran() != null ? pesan.getStatusPembayaran() : "belum_bayar");
            stmt.setInt(13, pesan.getIdTransaksi());
            return stmt.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    private PemesananOffline extractOffline(ResultSet rs) throws SQLException {
        ...
        poff.setBiayaJasa(rs.getBigDecimal("biaya_jasa"));
        poff.setTotalBiaya(rs.getBigDecimal("total_biaya"));
        poff.setStatusPembayaran(rs.getString("status_pembayaran"));
