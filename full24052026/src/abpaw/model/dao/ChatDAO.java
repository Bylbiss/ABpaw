...
public class ChatDAO {
    ...
    public boolean insert(Chat chat) {
        String sql = "INSERT INTO chat (id_dokter, id_pemilik, pesan, status_baca, pengirim) VALUES (?,?,?,?,?)";
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.setString(5, chat.getPengirim());
    ...
    public List<Chat> getChatBetween(int idDokter, int idPemilik) {
        List<Chat> list = new ArrayList<>();
        String sql = "SELECT * FROM chat WHERE id_dokter = ? AND id_pemilik = ? ORDER BY waktu ASC";
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, idDokter);
            stmt.setInt(2, idPemilik);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                list.add(extractChat(rs));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
