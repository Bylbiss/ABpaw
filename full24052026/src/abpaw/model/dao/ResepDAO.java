...
public class ResepDAO {
    ...
    public List<Resep> getResepByPemilik(int idPemilik) {
        List<Resep> list = new ArrayList<>();
        String sql = "SELECT r.* FROM resep_obat r "
                + "LEFT JOIN pemesanan_online po ON r.id_pemesanan = po.id_pemesanan AND r.tipe_pemesanan = 'Online' "
                + "LEFT JOIN pemesanan_offline poff ON r.id_pemesanan = poff.id_antrean AND r.tipe_pemesanan = 'Offline' "
                + "WHERE po.id_pemilik = ? OR poff.id_pemilik = ?";
        try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, idPemilik);
            stmt.setInt(2, idPemilik);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                list.add(extractResep(rs));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
