...
public class PembelianObatDAO {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (affected > 0) {
                try (ResultSet rs = stmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        pb.setIdPembelian(rs.getInt(1));
                        System.out.println("Generated ID: " + pb.getIdPembelian());
                    }
                }
                return pb;
            }
    ...
    public PembelianObat getById(int id) {
        String sql = "SELECT * FROM pembelian_obat WHERE id_pembelian = ?";
