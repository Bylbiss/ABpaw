...
public class RegisterView extends JFrame {
    ...
    private boolean validatePemilikData() {
        ...
        String error = authController.validatePemilikData(namaDepan, namaBelakang, username,
                password, noHp, email, alamat, false);
        if (error != null) {
            lblMessagePemilik.setText(error);
            return false;
        }
    ...
    private void register() {
        ...
        for (int i = 0; i < petsList.size(); i++) {
            ...
            PetPanel panel = petPanels.get(i);
            ...
            String alergi = panel.getAlergi();
            if (alergi != null && !alergi.isEmpty()) {
                boolean alergiSuccess = alergiController.addAlergi(pet.getIdPet(), alergi, newPemilik.getIdPemilik(), "pemilik");
                if (!alergiSuccess) {
                    System.err.println("Gagal menyimpan alergi untuk hewan " + pet.getNamaPet());
                }
            }
        }
    ...
    class PetPanel extends JPanel {
        ...
        public PetPanel(int nomor) {
            ...
            gbc.gridy = 8;
            gbc.gridx = 0;
            add(new JLabel("Alergi (opsional):"), gbc);
        ...
        public Pets getPets() {
            ...
            if (!usiaStr.isEmpty()) {
                try {
                    p.setUsia(Integer.parseInt(usiaStr));
            ...
            if (!beratStr.isEmpty()) {
                try {
                    p.setBerat(new BigDecimal(beratStr));
                } catch (NumberFormatException e) {
                    p.setBerat(null);
                }
            } else {
