...
public class RiwayatView extends JPanel {
    ...
    private void styleTable(JTable table) {
        Color pink = new Color(232, 62, 140);
        Color pinkMuda = new Color(255, 200, 220);

        table.getTableHeader().setBackground(pink);
    ...
    private void chatFromRiwayat() {
        ...
        if ("menunggu".equalsIgnoreCase(status)) {
            JOptionPane.showMessageDialog(this,
                    "Konsultasi masih dalam status menunggu.\n"
                    + "Silakan tunggu dokter mengubah status menjadi 'diproses' sebelum memulai chat.",
                    "Chat Belum Tersedia",
        ...
        if ("selesai".equalsIgnoreCase(status) || "batal".equalsIgnoreCase(status)) {
            JOptionPane.showMessageDialog(this,
                    "Konsultasi sudah selesai atau dibatalkan.\nChat tidak tersedia.",
                    "Chat Tidak Tersedia",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }

        // Status "diproses" atau "sudah bayar" diperbolehkan
