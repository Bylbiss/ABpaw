...
public class PemesananOnlineView extends JDialog {
    ...
    String spesiesDokter = dokter.getSpesiesHewan();
    
    if (spesiesDokter == null || spesiesDokter.trim().isEmpty()) {
        // Jika dokter tidak punya spesies terdaftar, tampilkan semua (fallback)
        petsYangSesuai = semuaPets;
    } else {
        List<String> daftarSpesies = Arrays.stream(spesiesDokter.split(","))
                .map(String::trim)
    ...
    for (Pets p : petsYangSesuai) {
        cbPet.addItem(p);
    }
    
    if (petsYangSesuai.isEmpty()) {
        JOptionPane.showMessageDialog(this, 
            "Tidak ada hewan peliharaan Anda yang sesuai dengan spesies yang ditangani Dr. " + dokter.getNamaLengkap() + 
            "\nDokter ini hanya menangani: " + spesiesDokter,
            "Tidak Ada Hewan Sesuai", 
